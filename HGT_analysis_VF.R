...
# PLOT ALL CHIMERA
pdf("/media/imateus/IvanHD2_Ubuntu/Noemie_HGT_Analysis/Results_TemplateC1.pdf", height = 10,width=14)
...
plotlines + geom_rect(aes(xmin=HGT_vf$start, xmax=HGT_vf$end, ymin=HGT_vf$ymin, ymax=HGT_vf$ymax),data=NULL,
  ...
  annotate("text", x = 400000, y = length(names(listOfFiles)) + 2, label = "A1552 template", fontface=2)   +
  annotate("text", x = 5000000, y = length(names(listOfFiles)) + 2, label = "Sa5Y template", fontface=2)   +
  annotate("text", x = Inserts$V4[1], y = 0, label = "Insert", fontface=2) +
  ggtitle("HGT treatment C1") + theme(plot.title = element_text(size = 20, face = "bold" )) + 
